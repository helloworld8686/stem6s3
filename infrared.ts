...
namespace makerbit {
  ...
  function enableIrMarkSpaceDetection(pin: DigitalPin) {
    ...
    pins.onPulsed(pin, PulseValue.High, () => {
      ...
      IrCondutctDuty(mark + space)    
      //control.raiseEvent(MICROBIT_MAKERBIT_IR_MARK_SPACE, mark + space);
    });
  }
  function IrCondutctDuty(duty:number)
  {
    let newCommand = irState.necIr.pushMarkSpace(duty)

    if ((0<newCommand)&&(newCommand < 255))
    {
